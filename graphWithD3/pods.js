...
function loadData() {
    ...
    xhttp.onreadystatechange=function() {
      if (this.readyState == 4 && this.status == 200) {
          ...
          data.pods.forEach(item => {
              map[item.metadata.ownerReferences[0].name] = item.metadata.ownerReferences[0].uid;
              kubeNodes.push({
                  "id": item.metadata.ownerReferences[0].uid,
          ...
          data.pods.forEach(item => {
            ...
            var nodeId = item.metadata.ownerReferences[0].uid;
            item.status.containerStatuses.forEach(item => {
                state = 'unknown';
    ...
    xhttp.open("GET", "http://localhost:60000/api/scrape/default?type=all&labelSelector&fieldSelector", true);
    xhttp.send();

    // $.getJSON("http://localhost:60000/api/scrape/default", null, function (data) {
    //     var map = {};
    //     // START kube nodes
...
function poll(type) {
    ...
    // oboe("http://localhost:60000/api/" + type + "?watch")
    // .done(function (evt) {
    //     console.log(evt);
