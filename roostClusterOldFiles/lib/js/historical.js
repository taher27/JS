...
// var color = d3.scale.ordinal().range(["#48A36D", "#80CEAA", "#80CCB3", "#7FC9BD", "#56AE7C",  "#64B98C", "#72C39B",  "#7FC7C6", "#809ECE", "#8897CE", "#8F90CD", "#9788CD", "#9E81CC", "#7EC4CF", "#7FBBCF", "#7FB1CF", "#80A8CE", "#AA81C5", "#B681BE", "#C280B7", "#CE80B0", "#D3779F", "#D76D8F", "#DC647E", "#E05A6D", "#E16167", "#E26962", "#E2705C", "#E37756", "#E38457", "#E39158", "#E29D58", "#E2AA59", "#E0B15B", "#DFB95C", "#DDC05E", "#DBC75F", "#E3CF6D", "#EAD67C", "#F2DE8A"]);  
var color = d3.scale.ordinal().range(["#48A36D", "Blue", "Red", "Green", "Black", "Brown", "Azure", "Ivory", "Teal", "Silver", "Purple", "Navy blue", "Pea green", "Gray", "Orange", "Maroon", "Charcoal", "Aquamarine", "Coral", "Fuchsia", "Wheat", "Lime", "Crimson", "Khaki", "Hot pink", "Magenta", "Olden", "Plum", "Olive", "Cyan", "#8897CE", "#8F90CD", "#9788CD", "#9E81CC", "#7EC4CF", "#7FBBCF", "#7FB1CF", "#80A8CE", "#AA81C5", "#B681BE", "#C280B7", "#CE80B0", "#D3779F", "#D76D8F", "#DC647E", "#E05A6D", "#E16167"]);
...
KEY = "LocalKey/79b6417f7c1bcf34f3e909928722c85b";
var flag = false; // to populate data only once in  dateData[];
...
function collectDataFromAllPods(pod) {
  ...
  var time = subMinutes(new Date(), 60);
  ...
  xmlhttp.onreadystatechange = function() {
            ...
            if(flag == false) {
              ...
              item.metrics.forEach(d => {
                if(item.tags.__name__ == "zbio_podcpu") {
                  dateData.push({date: new Date(d.timestamp)});
                }
              });
            }
            item.metrics.forEach(d => {
    ...
    xmlhttp.open('POST', `https://localhost:60001/api/podmetrics/kube-system?pod=${pod}&collection=podcpu&start=${time}&end=${t}`, true);
...
function collectPods() {
  ...
  flag = false; // making flag false so that it can populate dateData again.
      ...
      if (this.readyState === 4) {  
          if (this.status === 200) {
              ...
              if(allPods.length > 0) {
                ...
                setTimeout(()=> {
                  filterData(allPodsData, dateData) // will merge data of this 2 array and making it one(with one date and all pods value at that instance of time)
                } , 2000)
    ...
    xmlhttp.open('POST', `https://localhost:60001/api/scrape/kube-system?type=pods&labelSelector&fieldSelector`, true);
    xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xmlhttp.setRequestHeader('ZBIO_CLUSTER_KEY', KEY);
    xmlhttp.send();
}

function filterData(podsData, datesData) { 
  var AllPods = d3.nest()
  ...
  // 0:
  // key: "Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time)"
  // values: Array(7)
  // 0:
  // date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time) {}
  // storeData: {zbio-broker-group1-0: 8}
  // __proto__: Object
  // 1: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
  // 2: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
  // 3: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
  // 4: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
  // 5: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
  // 6: {date: Wed May 27 2020 22:29:21 GMT+0530 (India Standard Time), storeData: {…}}
