...
var svg = d3.select("body").select("#graphScreen").append("svg")
    .attr("width", width)
    .attr("height", height)
...
function resize() {
    ...
    // window.addEventListener("resize", function() { svg.attr("width", window.innerWidth).attr("height", window.innerHeight); });
...
function handleNewTaskEvent(evt) {
    ...
    if (isNull(serviceNode)) {
        // console.log("ERROR ERROR ERROR: Unable to create service node for serviceId: " + serviceId +" when creating task " + JSON.stringify(evt));
    }
...
$("#select-box").change(function () {
    ...
    // console.log("graph cleaned, ns: ", NAMESPACE);
    reloadGraph();
});

function clean(){
    d3.select("body").select("#graphScreen").selectAll("svg").remove();
    width = window.innerWidth;  // width and height of the SVG canvas
...
// add an SVG element inside the DOM's BODY element
 svg = d3.select("body").select("#graphScreen").append("svg")
    .attr("width", width)
    .attr("height", height)
    .call(d3.behavior.zoom().on("zoom", function () {
        svg.attr("transform", "translate(" + d3.event.translate + ")" + " scale(" + d3.event.scale + ")")
